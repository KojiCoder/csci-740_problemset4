...
\begin{solution}
    ...
    \]
    Since the transition probabilities $P_{ij}$ only depend on the state i, $\left\{X_n\right\}$ is a Markov chain.
    The limiting probabilities: 
...
\begin{solution}
    \begin{enumerate}
        % part a
        \item $X(t)$ is a CTMC when $\{F_n\}$ is memoryless and exponential.\\\\
        Rates $v_i$:
        \begin{itemize}
            \item $\lambda P_c$ for $v_0$
            \item $\lambda P_c \cdot p_i$ and $\mu_i$ for $v_i$ when $i > 0$
        \end{itemize}
        \begin{equation*}
            P_{ij} =
            \begin{bmatrix}
                -\lambda P_c & \lambda P_c \cdot p_1 & \lambda P_c \cdot p_2 & \lambda P_c \cdot p_3 & \dots \\
                \mu_1 & -\mu_1   & 0   & 0   & \dots \\
                0 & \mu_2  & -\mu_2   & 0   & \dots \\ 
                0 & 0   & \mu_3   & -\mu_3   & \dots \\
                0 & 0   & 0   & \mu_4   & \dots \\
